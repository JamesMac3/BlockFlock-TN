...
function Scene({ chapter, phase, records, infer }) {
  ...
  return <svg className="ix-scene" viewBox="0 0 640 360" role="img" aria-label={`Fictional street diagram. ${points.length} retained observations. ${chapter === 2 && infer ? 'Dashed links represent possible patterns, not proven journeys.' : ''}`}>
    ...
    {chapter === 2 && infer && PLACES.every((_, index) => records.filter(r => r.place === index).length > 1) && <path d="M135 220Q250 50 440 102L470 267Q270 315 135 220" className="ix-association" />}
    {PLACES.map((p, index) => <g key={p.name}>
      ...
      {points.filter(r => r.place === index).map((r, i) => <circle key={r.day} cx={p.x - 35 + (i % 6) * 13} cy={p.y - 33 - Math.floor(i / 6) * 13} r="4" className="ix-observation" style={{ animationDelay: `${chapter === 0 && phase === 1 ? (index === 0 ? 300 : 1800) : i * 45}ms` }} />)}
    </g>)}
  ...
  return <section className="ix-comparison" aria-label="Synchronized comparison of investigative order">
    ...
    <div className="ix-compare-columns">
      {[{label:'Targeted acquisition',order:['Reason','Obtain','Examine'],active:phase<2?-1:phase===2?0:2,steps:TARGETED_STEPS}, {label:'Persistent collection',order:['Collect','Retain','Reason','Search'],active:phase,steps:PERSISTENT_STEPS}].map((model,index)=><section key={model.label} className={`ix-model ix-model-${index}`}>
        ...
        <div className="ix-model-store"><span>EARLIER RECORD IN INVESTIGATORS’ SYSTEM</span><strong>{index === 0 ? (phase===3?'Only if obtained':'Not yet obtained') : phase===0?'Capture pending':'Already retained'}</strong></div>
      </section>)}
    </div>
...
export default function InversionExperience() {
  ...
  const shown = chapter === 1 ? retained : RECORDS.filter(r => r.day <= days);
  ...
  return <article className="ix-lesson">
    <header className="ix-hero">
      <div className="ix-eyebrow"><span className="ix-signal"/> THE INVERSION <span>AN INTERACTIVE FIELD GUIDE</span></div>
      ...
      <div className="ix-scope">Fictional plate and locations · Illustrative system · No live surveillance data</div>
    </header>
    ...
    <section className="ix-lab" aria-label={current.label}>
      ...
      <div className="ix-workbench">
        <div className="ix-visual">
          ...
          <Scene chapter={chapter} phase={phase} records={shown} infer={infer}/>
          <div className="ix-legend"><span><i/> Recorded sighting</span><span>Illustrative street layout</span></div>
          ...
          {chapter === 2 && <div className="ix-patterns">{['Evening location?', 'Morning routine?', 'Weekly visit?'].map((label,i)=><div key={label} className={infer && shown.filter(r=>r.place===i).length>1 ? 'ix-pattern-visible' : ''}><span>{PLACES[i].name}</span><strong>{infer && shown.filter(r=>r.place===i).length>1 ? label : 'More context needed'}</strong></div>)}</div>}
        </div>
        <aside className="ix-inspector" aria-label="What the records show">
          {chapter === 0 ? <>
            ...
            <div aria-hidden={phase === 0} className={`ix-record ${phase > 0 ? 'ix-record-visible' : ''} ${phase === 1 ? 'ix-record-capture' : ''}`}><span>TWO STORED SIGHTINGS</span><strong>ABC-123</strong><dl><div><dt>Cedar Road</dt><dd>7:42 PM</dd></div><div><dt>Market Street</dt><dd>7:44 PM</dd></div><div><dt>Reason at capture</dt><dd>None in this example</dd></div></dl></div>
            ...
            <div className="ix-inspector-label">{chapter === 1 ? 'CHANGE THE RETENTION WINDOW' : 'BUILD THE HISTORY'}</div>
            {chapter === 1 ? <><label className="ix-control" htmlFor="ix-retention">Keep observations for <strong>{retention === 0 ? 'no days' : `${retention} days`}</strong></label><input id="ix-retention" type="range" min="0" max="30" value={retention} onChange={e=>setRetention(Number(e.target.value))}/><div className="ix-range-labels"><span>No preservation</span><span>30 days</span></div><p className="ix-control-note">At day 29, an observation survives only if its age fits inside this window.</p></> : <><label className="ix-control" htmlFor="ix-history">Combine <strong>{days} {days===1?'day':'days'}</strong></label><input id="ix-history" type="range" min="1" max="28" value={days} onChange={e=>setDays(Number(e.target.value))}/><div className="ix-range-labels"><span>One moment</span><span>Four weeks</span></div><label className="ix-switch"><input type="checkbox" checked={infer} onChange={e=>setInfer(e.target.checked)}/> Show possible interpretations</label></>}
            <div className="ix-ledger-title"><span>RETRIEVABLE RECORDS</span><strong aria-live="polite">{shown.length}</strong></div>
            <div className="ix-ledger">{shown.length ? shown.slice(-5).map(r=><div key={r.day}><span>DAY {String(r.day).padStart(2,'0')}</span><span>{PLACES[r.place].name}</span><time>{r.time}</time></div>):<p className="ix-empty">No observations remain in this example.</p>}</div>
            {shown.length>5 && <p className="ix-table-note">Showing the latest 5 of {shown.length} records.</p>}
            <div className="ix-fact"><span>{chapter === 1 ? 'WHAT CHANGED?' : 'OBSERVATION ≠ EXPLANATION'}</span><p>{chapter === 1 ? 'The observation was fleeting. Keeping it made it available to a future search.' : 'A recurring location may suggest a routine. It does not establish who drove, why they were there, or whether an inference is correct.'}</p></div>
          </>}
        </aside>
      </div>
